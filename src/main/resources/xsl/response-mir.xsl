...
<xsl:stylesheet version="1.0"
    ...
    <div class="row result_searchline">
        ...
        <xsl:if test="normalize-space($isSolrSearchRequest) = 'true'">
          ...
          <div class="search_box">
            ...
            <xsl:variable name="condQuery"
                          select="/response/lst[@name='responseHeader']/lst[@name='params']/str[@name='condQuery']"/>

            <!-- Check if 'initialCondQuery' exists and extract its value if it does -->
            <xsl:variable name="initialCondQuery"
            ...
            <xsl:variable name="owner"
                          select="/response/lst[@name='responseHeader']/lst[@name='params']/str[@name='owner']"/>

            <!-- Check if 'version' exists and extract its value if it does -->
            <xsl:variable name="version"
                          select="/response/lst[@name='responseHeader']/lst[@name='params']/str[@name='version']"/>
                    ...
                    <xsl:choose>
                      ...
                      <xsl:when test="$lastTypeRequest = 'condQuery'">
                        <xsl:choose>
                          ...
                          <xsl:otherwise>
                            <xsl:value-of
                              select="concat($initialCondQuery, ' AND ', $decodedCurrentQryFromLastRequest)"/>
                          </xsl:otherwise>
                        </xsl:choose>
                      </xsl:when>
      ...
      <div class="col-12 col-sm-4">
        <form class="basket_form" action="{$ServletsBaseURL}MCRBasketServlet{$HttpSession}" method="post">
          ...
          <xsl:variable name="idNodes"
                        select="/response/result/doc|/response/lst[@name='grouped']/lst[@name='returnId']/arr[@name='groups']/lst/str[@name='groupValue']"/>
            ...
            <xsl:choose>
              <xsl:when test="count($idNodes)=0">
                ...
                <xsl:attribute name="class">
                  <xsl:value-of select="$buttonDefaultClasses"/> disabled
                </xsl:attribute>
              </xsl:when>
              <xsl:otherwise>
                <xsl:attribute name="class">
                  <xsl:value-of select="$buttonDefaultClasses"/>
                </xsl:attribute>
              </xsl:otherwise>
            </xsl:choose>
  ...
  <xsl:template match="doc" priority="10" mode="resultList">
    ...
    <xsl:variable name="completeHref">
      ...
      <xsl:value-of
        select="concat($href, '&amp;start=',$startPosition, '&amp;fl=id&amp;rows=1&amp;origrows=', $rows, '&amp;XSL.Style=browse', $q)"/>
    </xsl:variable>
    ...
    <xsl:variable name="derivid">
      <xsl:choose>
        <xsl:when
          test="count($derivates[count(arr[@name='derivateType']/str[text() = 'content'])&gt;0 and count(str[@name='derivateMaindoc']) &gt; 0])&gt;0">
          <xsl:value-of
            select="$derivates[count(arr[@name='derivateType']/str[text() = 'content'])&gt;0 and count(str[@name='derivateMaindoc']) &gt; 0][1]/str[@name='id']"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="''"/>
        </xsl:otherwise>
          ...
          <div class="hit_download_box">
            <xsl:variable name="viewerLink">
              <xsl:choose>
                ...
                <xsl:when test="$derivate/str[@name='iviewFile']">
                  <xsl:value-of
                    select="concat($WebApplicationBaseURL, 'rsc/viewer/', $derivid,'/', $derivate/str[@name='iviewFile'])"/>
                </xsl:when>
                <xsl:when
                  test="translate(str:tokenize($derivate/str[@name='derivateMaindoc'],'.')[position()=last()],'PDF','pdf') = 'pdf'">
                  <xsl:variable name="filePath"
                                select="concat($derivate/str[@name='id'],'/',mcr:encodeURIPath($derivate/str[@name='derivateMaindoc']),$HttpSession)"/>
                  <xsl:choose>
            ...
            <xsl:variable name="displayDerivateID">
              <xsl:choose>
                ...
                <xsl:when
                  test="count($derivates[arr[@name='derivateType']/str/text() = 'thumbnail' and count(str[@name='derivateMaindoc']) &gt; 0])&gt;0">
                  <xsl:value-of
                    select="$derivates[arr[@name='derivateType']/str/text() = 'thumbnail' and count(str[@name='derivateMaindoc']) &gt; 0][1]/str[@name='id']"/>
                </xsl:when>
                    ...
                    <xsl:choose>
                      <xsl:when test="not(mcrxsl:isCurrentUserGuestUser())">
                        <xsl:attribute name="data-iiif-jwt">
                          <xsl:value-of
                            select="concat($WebApplicationBaseURL, 'api/iiif/image/v2/thumbnail/', $identifier,'/full/!300,300/0/default.jpg')"/>
                        </xsl:attribute>
                      </xsl:when>
                      <xsl:otherwise>
              ...
              <xsl:for-each select="exslt:node-set($nameList)/arr/str[position() &lt;= 3]">
                ...
                <xsl:choose>
                  <xsl:when test="string-length($nameIdentifierAndType) &gt; 0">
                    ...
                    <xsl:variable name="nameQuery"
                                  select="concat('mods.nameIdentifier:', $nameIdentifierType, '\:', $nameIdentifier)"/>
                    <a
                  ...
                  <xsl:otherwise>
                    <a
                      href="{$ServletsBaseURL}solr/mods_nameIdentifier?q=mods.name:&quot;{$author_name}&quot;&amp;owner=createdby:{$owner}"
                      title="Suche nach allen Publikationen">
                      <xsl:value-of select="$author_name"/>
                    </a>
                  </xsl:otherwise>
              ...
              <xsl:choose>
                <xsl:when test="./str[@name='parentLinkText']">
                  <xsl:variable name="linkTo"
                                select="concat($WebApplicationBaseURL, 'receive/',./str[@name='parent'])"/>
                  <a href="{$linkTo}">
        ...
        <xsl:variable name="url">
          <xsl:choose>
            <xsl:when test="string-length($collection) &gt; 0">
              <xsl:choose>
                <xsl:when test="$layout = 'all'">
                  <xsl:value-of select="actionmapping:getURLforCollection('update-xml',$collection,true())"
                                xmlns:actionmapping="xalan://org.mycore.wfc.actionmapping.MCRURLRetriever"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="actionmapping:getURLforCollection('update',$collection,true())"
                                xmlns:actionmapping="xalan://org.mycore.wfc.actionmapping.MCRURLRetriever"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
              <xsl:choose>
                <xsl:when test="$layout = 'all'">
                  <xsl:value-of select="actionmapping:getURLforID('update-xml',$id,true())"
                                xmlns:actionmapping="xalan://org.mycore.wfc.actionmapping.MCRURLRetriever"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="actionmapping:getURLforID('update',$id,true())"
                                xmlns:actionmapping="xalan://org.mycore.wfc.actionmapping.MCRURLRetriever"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:otherwise>
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          <xsl:when test="$layout != '$'">
            <xsl:value-of
              select="concat($WebApplicationBaseURL,'/editor/editor-dynamic.xed',$HttpSession,'?id=',$id,'&amp;genre=article&amp;host=standalone')"/>
          </xsl:when>
          <xsl:otherwise>
  ...
  <xsl:template name="print.classiFilter">
    ...
    <div class="card">
      <xsl:variable name="classiDocument"
                    select="document(concat('xslStyle:items2options:classification:editor:-1:children:',$classId))"/>
      <div class="card-header" data-toggle="collapse-next">
        <h3 class="card-title">
      ...
      <xsl:variable name="filterHref">
        <xsl:choose>
          <xsl:when test="contains($RequestURL,'&amp;')">
            <xsl:value-of
              select="concat(substring-before($RequestURL, '&amp;'), $complete, substring-after($RequestURL, substring-before($RequestURL, '&amp;')))"/>
          </xsl:when>
          <xsl:otherwise>
          ...
          <xsl:variable name="dateFilter">
            <xsl:choose>
              <xsl:when test="contains(substring-after($RequestURL, '&amp;fq=mods.dateIssued'), '&amp;')">
                <xsl:value-of
                  select="concat($dateFilterHelper, '&amp;', substring-after(substring-after($RequestURL, '&amp;fq=mods.dateIssued'), '&amp;'))"></xsl:value-of>
              </xsl:when>
              <xsl:otherwise>
          ...
          <div class="dropdown-menu dropdown-menu-right stopAutoclose col-md-12 mir-date-arrowTop" role="menu">
            <div class="container-fluid">
              ...
              <div class="col-md-12 form-group dateContainer">
                ...
                <div class="col-md-4">
                  <input class="form-control" placeholder="YYYY" type="number" min="1000" max="2050"
                         style="padding: 0.1em"/>
                </div>
              </div>
              <div class="col-md-12 form-group">
